function SA(AST,CST)
{
        	...
        	function checkStmtList(node,scope)
        	{
        		...
        		if(node.name == "Assign")
        		{
        			...
        			else if(node.children[1].name == "!=" || node.children[1].name == "==")
        			{
                        ...
                        {
                            putMessage("Expected a Boolean type for boolval but got " + nodeType + " on line " + node.children[0].lineNum);
                            errorCount = errorCount + 1;
                        }
                ...
                if (node.name == "Print")
                {
                    ...
                    else
                    {
                        ...
                        {
                            var nodeType = symTable.cur.hashTable.findIdType(symTable.cur,node.children[0].name,node.lineNum);
                            //console.log(nodeType);
                            if (nodeType != false)
                            {
	...
	//putSA(createSymTable().toString());
    var holder;
    holder = createSymTable();
    if (errorCount < 1)
    {
        putSA(holder);
    }	
